...
public class MoveGroupCommand implements Command {
    ...
    public MoveGroupCommand(ArrayList<GraphicObject> gruppo, Point2D pos) {
        newPos = pos;
        this.gruppo = gruppo;
    ...
    @Override
    public boolean doIt() {
        for(GraphicObject object : gruppo) object.moveTo(newPos);
        System.out.println("Gruppo spostato");
        return true;
    }
    public boolean undoIt(){
        for (int i = 0; i < gruppo.size(); i++) {
            gruppo.get(i).moveTo(puntiVecchi[i]);
        }
